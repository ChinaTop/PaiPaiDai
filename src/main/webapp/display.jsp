...
<aside class="Hui-aside">
    ...
    <div class="menu_dropdown bk_2">
        ...
        <dl id="menu-product">
            ...
            <dd>
                <ul>
                    <li><a href="FundServlet?action=display" href="javascript:void(0)">基金管理</a></li>
            ...
            <table cellpadding="0" cellspacing="0" class="receivetab c666666">
                ...
                <c:forEach var="fund" items="${requestScope.funds}" varStatus="row">
                    <tr>
                        ...
                        <td>${fund.rate}</td>
                        <td>${fund.investamount}</td>
                        <td><fmt:formatDate value="${fund.investdate}" pattern="yyyy-MM-dd"/></td>
                        <td>${fund.deadline}</td>
                        <td>${fund.investman}</td>
                        <td>${fund.ifdisplay}</td>
                        <td>
                            <button id="bbb" type="button" onclick="xiajia(${fund.fundid})" class="btn-primary">
                                下架
                            </button>
                        </td>
                        <td>
                            <button type="button" onclick="shangjia(${fund.fundid},${fund.ifdisplay})" class="btn-primary">
                                上架
                            </button>
                        </td>
                    </tr>
...
<script type="text/javascript">
    ...
    function xiajia(fundid){
        window.location.href="AdminServlet?action=xiajia&fundid="+fundid
    }


    function shangjia(fundid,ifdisplay) {
        if (ifdisplay == 1) {
            window.location.href="addfund.jsp"
        } else if (ifdisplay == 0) {
            window.location.href = "AdminServlet?action=shangjia&fundid=" + fundid
        }
    }
